...
def get_pages(comic_name, issue_number):
    pages_list = []

    max_pages = cant_pages(comic_name, issue_number)
    
    for number in range(1, max_pages+1, 1):
        if number < 10:
            page_url = ("https://readcomicsonline.ru/uploads/manga/"+comic_name+"/chapters/"+issue_number+"/0"+str(number)+".jpg")
        else:
            page_url = ("https://readcomicsonline.ru/uploads/manga/"+comic_name+"/chapters/"+issue_number+"/"+str(number)+".jpg")
